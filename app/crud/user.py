...
def create_user_service(db: Session, user_in: UserCreate): 
    ...
    if existing_user:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="El email ya ha sido registrado")
...
def update_user_service(db: Session, user_id: int, updated_data: UserUpdate):
    user = get_user_by_id(db, user_id)
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No se encontro el usuario")
